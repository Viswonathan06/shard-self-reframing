...
This version uses AWS Bearer Token authentication.
"""
...
class BedrockBearerTokenClient:
    def __init__(self, region='us-west-2'):  # Changed to us-west-2 where Qwen is available
        self.region = region
        ...
        if not self.bearer_token:
            raise ValueError("AWS Bearer Token not found. Please set AWS_BEARER_TOKEN_BEDROCK environment variable")
    
    def invoke_model(self, model_id, body):
        """Invoke a model using AWS Bedrock Runtime with Bearer Token"""
        
        url = f'https://{self.host}/model/{model_id}/invoke'
...
def generate_o2_with_scratchpad(client, prompt_template, p1, o1, category, universal_guidelines, category_guidelines):
    ...
    # Extract the generated text - Qwen uses OpenAI ChatCompletions format
    generated_text = ""
    
    if isinstance(response, dict):
        # Qwen uses OpenAI ChatCompletions API format
        ...
        # Fallback to other formats
            ...
            if isinstance(response['output'], dict) and 'message' in response['output']:
            ...
            # If we can't find expected fields, return the whole response as string
...
def extract_scratchpad_and_response(generated_text):
    ...
    if '<scratchpad>' in generated_text and '</scratchpad>' in generated_text:
        ...
        # Fallback parsing - look for the 4-step structure
        if 'STEP 1 - HARM ASSESSMENT' in generated_text:
            # Try to separate scratchpad from final response
            ...
            for line in lines:
                if 'STEP' in line and 'HARM ASSESSMENT' in line:
                    step_count = 1
                    in_scratchpad = True
                    scratchpad_lines.append(line)
                elif 'STEP' in line and ('LEGITIMATE NEED' in line or 'NEED' in line):
                    step_count = 2
                    scratchpad_lines.append(line)
                elif 'STEP' in line and ('MINIMAL RESTRICTION' in line or 'RESTRICTION' in line):
                    step_count = 3
                    scratchpad_lines.append(line)
                elif 'STEP' in line and ('CARING ENGAGEMENT' in line or 'ENGAGEMENT' in line):
                    step_count = 4
                    scratchpad_lines.append(line)
                elif in_scratchpad and step_count > 0 and step_count < 4:
                    scratchpad_lines.append(line)
                elif in_scratchpad and step_count == 4:
                    # Still in step 4
                    if line.strip() and not any(phrase in line.lower() for phrase in ['i understand', 'i can help', 'here\'s', 'let me']):
                        scratchpad_lines.append(line)
                    else:
                        # This looks like the start of the actual response
                        in_scratchpad = False
            ...
            # Complete fallback - assume it's all O2 response
...
def main():
    ...
    try:
        client = BedrockBearerTokenClient()
        print("✅ AWS Bedrock client initialized with Bearer Token")
    except ValueError as e:
        print(f"❌ {e}")
        print("Please set your AWS Bearer Token:")
        print("export AWS_BEARER_TOKEN_BEDROCK=your_bearer_token")
        return
    
    # Load prompt template
    if not os.path.exists(args.prompt_template):
        print(f"❌ Prompt template not found: {args.prompt_template}")
        return
        ...
        for i, line in enumerate(f):
            ...
            try:
                ...
                output_record = {
                    ...
                    'model_used': 'qwen3-coder-480b-bedrock-bearer',
                ...
                error_record = {
                    ...
                    'category': category,
                    'error': str(e),
                    'model_used': 'qwen3-coder-480b-bedrock-bearer-error'
                }
